...
export default function shortDescription(input) {
  const error = Error('Такого игрока нет');
  const emojies = {
    Bowman: '0x1F620',
    Swordsman: '0x1F621',
    Magician: '0x1F9D0',
  ...
  const out = `${typeOfPlayer} ${input.name.charAt(0)}(${input.level})${attackEmoji}${input.attack} ${defenceEmoji}  ${input.defence} ${healthEmoji} ${input.health}`;
